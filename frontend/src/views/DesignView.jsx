...
// ── Combination scoring ───────────────────────────────────────────────────────
// Scores all 27 (3×3×3) combinations and returns top 3.
// Scoring metric:
//   score = net_10yr_benefit - (capex × capex_penalty)
// where net_10yr_benefit = 10yr cumulative savings - capex.
// Combinations with earlier break-even are rewarded with a bonus.
// This balances maximum savings against lower upfront cost.
function scoreCombinations(systemDesign) {
  if (!systemDesign) return []
  const { panels, inverters, batteries, profile, requirements, capex_breakdown, simulation_config } = systemDesign
  if (!panels?.length || !inverters?.length || !batteries?.length) return []

  const base    = capex_breakdown
  const prof    = profile
  const req     = requirements
  const tariff  = simulation_config?.tariff

  const annualDemandSaving = prof.user_type === 'facility'
    ? (computeDemandSavings(prof.peak_load_kw)?.annual_demand_saving_sar ?? 0)
    : 0
  ...
  for (const panel of panels) {
    for (const inv of inverters) {
      for (const bat of batteries) {
        const panelCost = panel.panels_cost_sar  ?? base.panels_sar
        const invCost   = inv.inverter_cost_sar   ?? base.inverter_sar
        const batCost   = bat.battery_cost_sar    ?? base.battery_sar
        const capex     = panelCost + invCost + batCost + base.protection_sar + base.bos_sar

        const fin = computeFinancials({
          arrayKwp:          panel.actual_kwp ?? req.pv_kwp_required,
          ghi:               prof.ghi,
          performanceRatio:  req.performance_ratio || 0.655,
          tariff,
          gridScenario:      prof.grid_scenario,
        ...
        const net10     = fin.net_10yr_benefit_sar
        const bey       = fin.break_even_year ?? 11
        // Reward earlier break-even: each year earlier = +5% of net10 bonus
        const beyBonus  = net10 > 0 ? net10 * ((11 - bey) * 0.05) : 0
        const score     = net10 + beyBonus

        combos.push({
          panel, inv, bat,
          capex, fin, score,
          net10, bey,
          yr1Savings: fin.year_1_savings_sar,
        })
      }
    }
  }
...
function BestCombinations({ systemDesign, onApply }) {
  const [open, setOpen] = useState(false)
  const combos = useMemo(() => scoreCombinations(systemDesign), [systemDesign])
  if (!combos.length) return null

  const RANK_LABELS = [
    { label: '🥇 Best overall', bg: '#FFF8E1', border: '#F59E0B', text: '#92400E' },
    { label: '🥈 Best value',   bg: '#F0FBF7', border: '#1D9E75', text: '#085041' },
    { label: '🥉 Best budget',  bg: '#F3F0FF', border: '#534AB7', text: '#3C3489' },
  ]

  const fmtSar = v => v >= 1_000_000
    ? `${(v / 1_000_000).toFixed(2)}M SAR`
    : `${Math.round(v).toLocaleString()} SAR`

  return (
    <div style={{ marginBottom: 20, border: '1.5px solid #F59E0B', borderRadius: 10, overflow: 'hidden', background: '#FFFDF5' }}>
      <button
        onClick={() => setOpen(o => !o)}
        style={{ width: '100%', padding: '13px 18px', background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', textAlign: 'left' }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 18 }}>⚡</span>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#111827' }}>Smart Combination Recommendations</div>
            <div style={{ fontSize: 11, color: '#6B7280', marginTop: 1 }}>
              AI-ranked top 3 combinations from all 27 possible (performance + budget optimised)
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 10, fontWeight: 700, background: '#FEF3C7', color: '#92400E', padding: '2px 8px', borderRadius: 10 }}>
            NEW · {combos.length} combos ranked
          </span>
      ...
      {open && (
        <div style={{ borderTop: '1px solid #FDE68A', padding: '14px 18px' }}>
          <p style={{ fontSize: 11, color: '#6B7280', marginBottom: 14, lineHeight: 1.6 }}>
            Scored across all 27 combinations using: <strong>10-year net benefit + break-even bonus</strong>.
            Each combination's score rewards high savings <em>and</em> an early payback date.
            Click <strong>Apply</strong> on any combo to auto-select those components below.
          </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {combos.map((c, i) => {
              ...
              return (
                <div key={i} style={{ background: rank.bg, border: `1px solid ${rank.border}`, borderRadius: 8, padding: '14px 16px' }}>
                  {/* Rank badge + apply button */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: rank.text, background: '#fff', border: `0.5px solid ${rank.border}`, padding: '2px 9px', borderRadius: 8 }}>
                      {rank.label}
                    </span>
                    <button
                      onClick={() => { onApply(c.panel, c.inv, c.bat); setOpen(false) }}
                      style={{ fontSize: 12, fontWeight: 600, background: rank.border, color: '#fff', border: 'none', borderRadius: 7, padding: '6px 16px', cursor: 'pointer' }}
                    >
                      Apply this combo →
                    </button>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 10 }}>
                    {[
                      { icon: '☀️', label: 'PV Panel', name: `${c.panel.brand} ${c.panel.model}`, detail: `${c.panel.actual_kwp} kWp · ${fmtSar(c.panel.panels_cost_sar)}` },
                      { icon: '⚡', label: 'Inverter', name: `${c.inv.brand} ${c.inv.model}`, detail: `${c.inv.actual_kw ?? c.inv.capacity_kw} kW · ${fmtSar(c.inv.inverter_cost_sar)}` },
                      { icon: '🔋', label: 'Battery', name: `${c.bat.brand} ${c.bat.model}`, detail: `${c.bat.actual_kwh ?? c.bat.capacity_kwh} kWh · ${fmtSar(c.bat.battery_cost_sar)}` },
                    ].map(item => (
                      <div key={item.label} style={{ background: '#fff', borderRadius: 6, padding: '8px 10px', border: `0.5px solid ${rank.border}40` }}>
                        <div style={{ fontSize: 10, color: '#9CA3AF', marginBottom: 2 }}>{item.icon} {item.label}</div>
                        <div style={{ fontSize: 11, fontWeight: 600, color: '#111827', lineHeight: 1.3 }}>{item.name}</div>
                        <div style={{ fontSize: 10, color: '#6B7280', marginTop: 2 }}>{item.detail}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
                    {[
                      { label: 'Total CAPEX',      value: fmtSar(c.capex),       color: '#374151' },
                      { label: 'Year 1 savings',   value: fmtSar(c.yr1Savings),  color: '#1D9E75' },
                      { label: '10yr net benefit', value: fmtSar(c.net10),       color: c.net10 >= 0 ? '#1D9E75' : '#A32D2D' },
                      { label: 'Break-even',       value: c.bey <= 10 ? `Year ${c.bey}` : '>10 yrs', color: c.bey <= 7 ? '#1D9E75' : '#BA7517' },
                    ].map(m => (
                      <div key={m.label} style={{ background: '#fff', borderRadius: 6, padding: '6px 8px', textAlign: 'center', border: `0.5px solid ${rank.border}40` }}>
          ...
          <div style={{ fontSize: 10, color: '#9CA3AF', marginTop: 12, lineHeight: 1.5 }}>
            Scoring formula: 10yr net benefit + break-even bonus (each year earlier than 10 = +5% of net benefit).
            Indicative estimates — same assumptions as the financial report below.
          </div>
        </div>
      )}
    </div>
        ...
        {systemDesign && !designLoading && (designStage === 'base' || designStage === 'final') && (
          <>
            ...
            <div style={S.card}>
              ...
              {/* Smart combination recommendations — scored across all 27 combos */}
              <BestCombinations
                systemDesign={systemDesign}
                onApply={(panel, inv, bat) => {
